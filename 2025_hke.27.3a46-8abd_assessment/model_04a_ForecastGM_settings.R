################################################################################
#  hke.27.3a46-8abd_assessment : short-term forecast GM settings               #
#------------------------------------------------------------------------------#
...
# model_02a_ForecastGM_settings.R - STF general settings (with geometric mean recruitment)
# ~/*_hke.27.3a46-8abd_assessment/model_02a_ForecastGM_settings.R
...
dir.fmult       <- file.path(stfGM_path,"stf_files")
dir.fored       <- file.path(stfGM_path, "info_models")

dir.create(path = stfGM_path, showWarnings = TRUE, recursive = TRUE)
...
hist.rec <- as.data.frame(ass.sum$recruits) %>% filter(Yr %in% 1990:(year_inter-1)) %>% .[,1]  #! since 1978 or 1990?
...
for (i in 1:length(fmult)) {
  ...
  r4ss::SS_writeforecast(fore, dir = dir.forecastTAC, file = paste0("forecastGM_",Fmult_names[i], ".ss"), 
