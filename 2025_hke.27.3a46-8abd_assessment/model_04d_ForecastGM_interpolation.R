################################################################################
#  hke.27.3a46-8abd_assessment : short-term forecast GM final values           #
#------------------------------------------------------------------------------#
...
# model_02d_ForecastGM_interpolation.R - STF final (with geometric mean recruitment) values (after interpolation)
# ~/*_hke.27.3a46-8abd_assessment/model_02d_ForecastGM_interpolation.R
...
tabledir_stf <- file.path(stfGM_path, "table")
...
write.csv (stfTab, file.path("model", "sum_advice", "catchoptiontableGM_runs.csv"), row.names = FALSE)
...
#==============================================================================
# CATCH OPTION TABLE (with interpolation)                                 ----
#==============================================================================
...
df2 <- df1 %>% mutate(Fland = !!as.name(paste0("F",catYr)) * !!as.name(paste0("Landings",catYr))/!!as.name(paste0("Catches",catYr)),
                      Fdisc = !!as.name(paste0("F",catYr)) * !!as.name(paste0("Discards",catYr))/!!as.name(paste0("Catches",catYr)), 
                      SSBchg = (!!as.name(paste0("SSB",ssbYr))/SSBint-1)*100, 
  ...
  select(paste0("Catches",catYr), paste0("Landings",catYr), paste0("Discards",catYr), 
         paste0("F",catYr), Fland, Fdisc, 
         paste0("SSB",ssbYr), SSBchg, ADVchg, TACchg, paste0("pBlim_",ssbYr)) %>% 
  rename(!!paste0("Fland",year_inter+1) := Fland, !!paste0("Fdisc",year_inter+1) := Fdisc)

write.csv (df2, file.path("model","sum_advice","catchoptiontableGM_final.csv")) # keep row.names, as correspondo to scenarios
...
write.csv (df2, file.path("model","sum_advice","catchoptiontableGM_final_ICESround.csv")) # keep row.names, as correspondo to scenarios
...
## Plots -----------------------------------------------------------------------
png(height=600, width=600,  file=file.path("model", "sum_report", "ShortTermProjGM.png")) #file.path(plotdir_stf, "plot_stf.png")
